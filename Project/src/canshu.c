...
//避障方式路径规划： 
uint8_t turn[20]={4,4,4,3,3,4,4,4,3,4,4,4,4,4,4,4,4,4,4,4};  //路径规划： 1 避障左转； 2 避障右转； 3 左切入； 4 右切入；
...
uint8_t speedzhuanwan=  15;        //寻找信标转弯速度
uint8_t speedjiasu=     38;            //远离信标 直线加速
uint8_t speedxinjiansu= 16;        //靠近信标 进行减速
uint8_t speedxinjia=    28;          //靠近信标 速度太慢 而加速
uint8_t speeddazhuan=   20;         //找不到信标大转弯速度
...
//舵机打角参数：
uint16_t anglemid=510;//舵机中值
uint16_t angleleft=609;//舵机左极限 
uint16_t angleright=415;//舵机右极限 

uint16_t hide_left=560;//避障的左转打角
uint16_t hide_right=460;//避障的右转打角

uint16_t acc_left=525;//加速段舵机打角左限
uint16_t acc_right=495;//加速度段舵机打角右限

uint16_t hide2_left=545;//信标避障的左转打角      切过信标不使用
uint16_t hide2_right=475;//信标避障的右转打角

float anglek=0.43;//打角系数0.48
 int16_t Kd=12;//舵机pd微分系数
 
//避障：
uint16_t door_7620=65;//7620避障像素点个数阀值
uint8_t h7620=45;//7620避障距离

uint16_t delay1=15;//普通避障延时
uint16_t delay2=8;//信标避障延时
...
uint16_t xinbiaobizhang_h=16;//信标避障行坐标
...
uint16_t acc_stop=30;//加速结束时的行坐标  
