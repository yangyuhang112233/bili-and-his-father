...
//避障方式路径规划：
uint8_t turn[20]={3,3,3,4,4,4,3,3,4,3,3,4,4, 3,3,3,3,3,3,3};  //路径规划： 1 避障左转； 2 避障右转； 3 左切入； 4 右切入；
uint8_t turn_route[20]={3,3,3,4,4,4,4,4,3,3,4,4,3, 3,3,3,3,3,3,3};	//认为设定的lookfor 车辆转向;
//
//uint8_t turn[20]={3,4,3,3,3,3,3,3,3,3,3,3,3, 3,3,3,3,3,3,3};  //路径规划： 1 避障左转； 2 避障右转； 3 左切入； 4 右切入；
//uint8_t turn_route[20]={3,3,4,4,4,4,1,3,4,4,4,4,4, 3,3,3,3,3,3,3};	//认为设定的lookfor 车辆转向;
//	
//uint8_t turn[20]={4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4};  //路径规划： 1 避障左转； 2 避障右转； 3 左切入； 4 右切入；
//uint8_t turn_route[20]={4,3,4,4,4,4,4,4,4,4,3,4,3,3,3,3,3,3,3};	//认为设定的lookfor 车辆转向;
...
//uint8_t speeddazhuan=   20;         //找不到信标 大转弯速度    
...
float pianchak=1.0;//边缘切过时的偏差计算系数
...
//避障行坐标 低速好用
uint16_t xinbiaobizhang_h=34;//信标避障行坐标   
float k11=1;
