...
#define  DMA_IRQ_EN(DMA_CHn)    NVIC_EnableIRQ((IRQn_Type)(DMA0_IRQn + DMA_CHn));   //允许DMA传输完成中断
#define  DMA_IRQ_DIS(DMA_CHn)   NVIC_DisableIRQ((IRQn_Type)(DMA0_IRQn + DMA_CHn));   //禁止DMA传输完成中断

#define  DMA_IRQ_CLEAN(DMA_CHn) DMA0->INT|=(DMA_INT_INT0_MASK<<DMA_CHn)           //清除中断标志位

#define  DMA_EN(DMA_CHn)        DMA0->ERQ |= (DMA_ERQ_ERQ0_MASK<<(DMA_CHn))       //使能DMA请求
#define  DMA_DIS(DMA_CHn)       DMA0->ERQ &=~(DMA_ERQ_ERQ0_MASK<<(DMA_CHn))       //禁止DMA请求
